...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk)
{
     ...
     * subtracted from s2.
     **********************************************/
    sp_scope_t sc = sp_begin("keypair:polyveck_expand");
    polyveck_expand(&a, rhoprime);
    sp_end(&sc);

reject:
    // Sample secret vectors s1 and s2
    sc = sp_begin("keypair:polyvecmk_uniform_eta");
    ...
    // b = a + A0 * s1 + s2 mod q
    s1hat = s1;
    sc = sp_begin("keypair:polyvecm_ntt");
    polyvecm_ntt(&s1hat);
    sp_end(&sc);
    sc = sp_begin("keypair:polymatkm_pointwise_montgomery");
    polymatkm_pointwise_montgomery(&b, A, &s1hat);
    sp_end(&sc);
    sc = sp_begin("keypair:polyveck_invntt_tomont");
    polyveck_invntt_tomont(&b);
    sp_end(&sc);
    sc = sp_begin("keypair:polyveck_add");
    polyveck_add(&b, &b, &s2);
    sp_end(&sc);
    sc = sp_begin("keypair:polyveck_add");
    polyveck_add(&b, &b, &a);
    sp_end(&sc);
    sc = sp_begin("keypair:polyveck_freeze");
    polyveck_freeze(&b);
    sp_end(&sc);
...
reject:
	...
	int64_t squared_singular_value = polyvecmk_sqsing_value_min(sum);
