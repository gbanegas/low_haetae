...
int main(void) {
	...
	hal_send_str("=== Testing crypto_sign_keypair_low_stack ===\n\n");

	sc = sp_begin("keypair_low_stack");
	crypto_sign_keypair_low_stack(pk, sk);
	sp_end(&sc);
	hal_send_str("KeyPair (low_stack) OK!\n\n");

	hal_send_str("=== Testing crypto_sign_keypair_streamed_v2 ===\n\n");
	sc = sp_begin("keypair_streamed_v2");
	crypto_sign_keypair_streamed_v2(pk, sk);
	sp_end(&sc);
	hal_send_str("KeyPair (streamed_v2) OK!\n\n");
	for (int i = 0; i < 1; i++) {
