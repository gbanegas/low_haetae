...
/* User LEDs: PD12..PD15. Choose one. */
#define LED_PORT        GPIOD
#define LED_PIN         GPIO12

/* UART: USART2 on PA2/PA3 (use external USB-UART dongle). */
#define SERIAL_GPIO     GPIOA
#define SERIAL_USART    USART2
#define SERIAL_PINS     (GPIO2 | GPIO3)
...
/* 24 MHz benchmark clock (keeps FLASH wait-states low). */
const struct rcc_clock_scale benchmarkclock = {
  .pllm = 8,            /* VCOin  = HSE/PLLM = 8MHz/8 = 1MHz */
  .plln = 192,          /* VCOout = VCOin*PLLN = 192MHz */
  .pllp = 8,            /* PLLCLK = VCOout/PLLP = 24MHz */
  .pllq = 4,            /* PLL48CLK = 48MHz (if present/used) */
  .pllr = 0,
  .hpre = RCC_CFGR_HPRE_DIV_NONE,
  .ppre1 = RCC_CFGR_PPRE_DIV_2,
  .ppre2 = RCC_CFGR_PPRE_DIV_NONE,
...
static void clock_setup(enum clock_mode clock)
{
  ...
  default:
    ...
    /* F411 max is 100MHz; 84MHz preset is a safe default. */
...
#if defined(DISCOVERY_BOARD) || defined(NUCLEO_BOARD) || defined(CW_BOARD) || defined(STM32F411E_DISCO)
  gpio_set_output_options(SERIAL_GPIO, GPIO_OTYPE_PP, GPIO_OSPEED_100MHZ, SERIAL_PINS);
