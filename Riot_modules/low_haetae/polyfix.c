...
void polyfix_round_inplace(polyfix *a) {
    unsigned i;
    for (i = 0; i < N; ++i) {
        a->coeffs[i] = fix_round(a->coeffs[i]);
    }
}
...
uint16_t polyfixveclk_two_pass_sample_hyperball(polyfixvecl *y1, polyfixveck *y2, uint8_t *b,
    ...
    /*
     * Low-stack two-pass hyperball sampler.
     *
     * Pass 1: compute the squared norm of the unscaled Gaussian vector
     *         without storing (L+K)N samples.
     * Pass 2: regenerate the same Gaussian stream (same seed+nonce sequence)
     *         and write scaled coefficients directly into (y1,y2).
     */

    /* Small scratch buffers (BSS), reused across calls (not thread-safe). */
    static uint64_t tmp_samples[N + 1];
    static uint8_t  tmp_signs[(N + 7) / 8];

    do {
        const uint16_t start_nonce = ni;
        uint16_t end_nonce = ni;

        /* ---- pass 1: norm-only (discard samples) ---- */
        sqsum.limb48[0] = 0;
        sqsum.limb48[1] = 0;

        sample_gauss_N(tmp_samples, tmp_signs, &sqsum, seed, end_nonce++, N + 1);
        sample_gauss_N(tmp_samples, tmp_signs, &sqsum, seed, end_nonce++, N + 1);
        for (i = 2; i < L + K; i++) {
            sample_gauss_N(tmp_samples, tmp_signs, &sqsum, seed, end_nonce++, N);
        }

        /* Compute scaling factor (same arithmetic as the baseline sampler). */
        ...
        /* ---- pass 2: regenerate and write scaled coefficients ---- */
        {
            fp96_76 dummy;
            uint16_t regen_nonce = start_nonce;

            for (i = 0; i < L + K; i++) {
                const size_t len = (i < 2) ? (N + 1) : N;

                dummy.limb48[0] = 0;
                dummy.limb48[1] = 0;
                sample_gauss_N(tmp_samples, tmp_signs, &dummy, seed, regen_nonce++, len);
                ...
                if (i < L) {
                    ...
                    const unsigned int ii = i - L;
                    for (j = 0; j < N; j++) {
                        y2->vec[ii].coeffs[j] = fixpoint_mul_rnd13(
                            tmp_samples[j], &sqsum,
                            (tmp_signs[j / 8] >> (j % 8)) & 1);
                    }
                }
        ...
        /* Advance nonce once per attempt (pass 2 is regeneration). */
        ni = end_nonce;
...
uint16_t polyfixveclk_one_pass_sample_hyperball(polyfixvecl *y1, polyfixveck *y2, uint8_t *b,
                                       const uint8_t seed[CRHBYTES],
                                       const uint16_t nonce) {
    uint16_t ni = nonce;
...
uint16_t polyfixveclk_sample_hyperball(polyfixvecl *y1, polyfixveck *y2, uint8_t *b,
                                       ...
                                       const uint16_t nonce) {
#if SAMPLER == 1
    return polyfixveclk_one_pass_sample_hyperball(y1, y2, b, seed, nonce);
#else
    return polyfixveclk_two_pass_sample_hyperball(y1, y2, b, seed, nonce);
#endif
}
